  ...
  <div class="container">
    ...
    <div class="nav-page">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item :to="{ path: '/index/pageuserlist' }" :replace="true">用户列表</el-breadcrumb-item>
      ...
      <div class="right">
        <div style="margin-right: 20px;margin-top: 5px">
          <el-form label-width="120px">
            <el-form-item label="信号发布：">
              <el-select v-model="signalok" placeholder="请选择">
                <el-option
                  v-for="item in signaloks"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
            ...
            <el-form-item label="模拟跟单：">
              <el-select v-model="accountok" placeholder="请选择">
                <el-option
                  v-for="item in accountoks"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
  ...
  export default {
    ...
    methods: {
      initData(){
        ...
        this.signalok = commodity.is_signal;
        this.accountok = commodity.is_follower_demo;
        this.serverid = commodity.userid;
        this.btnTx = "确认修改";
      },
      closePage(){
        this.$router.go(-1);
      ...
      submitData(){
        if (utils.isEmpty(this.signalok)) {
          this.$message("信号未选择!");
          return;
        }
        if (utils.isEmpty(this.accountok)) {
          this.$message("跟单未选择!");
          return;
        }
        this.updatePdData();
      ...
      updatePdData(){
        ...
        var _this = this
        var paras = {
          action: "sys_user_modify",
          userid: userid,
          random: random,
          randomkey: randomkey,
          modified_userid: _this.serverid,
          is_signal: _this.signalok,
          is_follower_demo: _this.accountok,
        }
        $.ajax({
          traditional: true,
          url: api.APPURL,
