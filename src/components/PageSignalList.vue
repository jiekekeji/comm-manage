  ...
  <div class="container" id="container-orderlist">
    ...
    <div class="option-area">
      <ul class="option-item">
        ...
        <li>
          <span>信号名称:</span>
          <el-input v-model="signalname" style="display: inline-block;width: 180px"></el-input>
        </li>
      </ul>
    </div>
    <!--数据列表的操作区域二-->
    ...
    <div class="data-list">
      <el-table :data="result.datas" :height="tableHeight" border style="width: 100%">
        <el-table-column fixed prop="signalid" label="信号ID" min-width="150">
        </el-table-column>
        <el-table-column prop="signalname" label="信号名" min-width="160">
        </el-table-column>
        ...
        <el-table-column prop="balance" label="净值" min-width="120">
        </el-table-column>
        <el-table-column prop="equity" label="余额" min-width="120">
        </el-table-column>
        <el-table-column prop="profitfactor" label="利润因子" min-width="120">
        </el-table-column>
        <el-table-column prop="avgpips" label="平均点" min-width="120">
        </el-table-column>
        <el-table-column prop="maxdd" label="最大回撤" min-width="120">
        </el-table-column>
        <el-table-column prop="weeks" label="时长(周)" min-width="120">
        </el-table-column>
        <el-table-column prop="followers" label="跟随者" min-width="120">
        </el-table-column>
        <el-table-column prop="followeramt" label="跟随AMT" min-width="120">
        </el-table-column>
        <el-table-column label="vps" min-width="120">
          <template scope="scope">
            <p v-bind:style="{ color: genVps(scope.row.vps)}">
              {{scope.row.vps}}</p>
          </template>
        </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      stopSignal(item){
        let r = confirm("确认要停止该信号" + item.signalid + "吗？")
        if (r == true) {
          let userid = utils.getToken().userid;
          let random = utils.getRandom();
          let randomkey = utils.getRandomkey(random);
